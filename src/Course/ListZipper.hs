...
setFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
setFocus val (ListZipper l _ r) = ListZipper l val r
...
findLeft p (ListZipper l x r) = case break p l of
   (_, Nil) -> IsNotZ
   (newR, h :. t) -> IsZ (ListZipper t h (reverse newR ++ x :. r))
  -- error "todo: Course.ListZipper#findLeft"
-- using let (a, b) = something to unpack values is nice
...
findRight p (ListZipper l x r) = case break p r of
    (_, Nil) -> IsNotZ
    (newL, h :. t) -> IsZ (ListZipper (reverse newL ++ x :. l) h t)
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop z = case findLeft (const True) z of
  (IsZ z') -> z'
  IsNotZ -> let h :. t = reverse (toList z) in
    ListZipper t h Nil
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop z = case findRight (const True) z of
  (IsZ z') -> z'
  IsNotZ -> let h :. t = toList z in
    ListZipper Nil h t
...
moveLeftN' n z@(ListZipper l _ _)
  | n == 0 = Right . id $ z
  | n > 0 = if n > length l then Left $ length l else moveLeftN' (n - 1) $ moveLeftLoop z
...
moveRightN' n z@(ListZipper _ _ r)
  | n == 0 = Right . id $ z
  | n > 0 = if n > length r then Left $ length r else moveRightN' (n - 1) $ moveRightLoop z
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n z = moveRightN n -<< fromList (toList z)
...
-- prop> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end (ListZipper l x r) = let (h :. t) = reverse r ++ x :. l in
  ListZipper t h Nil
...
instance Apply ListZipper where
  (ListZipper lf xf rf) <*> (ListZipper l x r) =
    ListZipper (zipWith ($) lf l) (xf x) (zipWith ($) rf r)
    -- error "todo: Course.ListZipper (<*>)#instance ListZipper"
  -- (zipWith ($)) seems like a useful pattern for "zip with function
  -- application"
...
-- The `unfoldr` is the tricky part here. Basically `unfoldr` takes
-- something and then makes a list from it. So we need to make the
-- left and right sides.  The function that we pass to `unfoldr`
-- basically takes whatever you're generating the list from, and then
-- turns it into a tuple of `(an elt of the list, the state of the
-- generator after making that elt)`

-- In this case, for the left side, we want to `moveLeft` to traverse
-- over the ListZipper. We then need to look at that movedLeft
-- version, apply `f` to it, and stick it into that `(b, ListZipper
-- a)` format.  Once we have a real idea of what we want to do it's
-- not too hard to write the function.
...
instance Extend MaybeListZipper where
  ...
  f <<= (IsZ z) = IsZ (f . IsZ <<= z)
    -- error "todo: Course.ListZipper (<<=)#instance MaybeListZipper"
 -- `f` wants a `MaybeListZipper` so we need to do `IsZ` first.
...
-- We can easily make `f (List b)` and `f b`. We want to stick those
-- into an `f ListZipper b` instead of an `ListZipper f b` (besides,
-- that wouldn't even work, we need `List (f b)`).  So we lift the
-- constructor into `f`.
...
instance Traversable MaybeListZipper where
  ...
  -- traverse will give us the `ListZipper` in an `f` - we need to
  -- `<$>` into it.
